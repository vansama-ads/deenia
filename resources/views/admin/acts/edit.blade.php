            ...
            <form action="{{ route('admin.acts.update', $act->id) }}" method="POST">
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <input type="text" id="name" name="name" placeholder="Contoh: Act 1 - Pengenalan" 
                        ...
                        value="{{ old('name', $act->name) }}" required>
                    @error('name')
                        <span style="color: #dc3545; font-size: 12px; margin-top: 5px; display: block;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <textarea id="description" name="description" placeholder="Masukkan deskripsi act..." 
                        ...
                        rows="5">{{ old('description', $act->description) }}</textarea>
                    @error('description')
                        <span style="color: #dc3545; font-size: 12px; margin-top: 5px; display: block;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <input type="number" id="order_number" name="order_number" placeholder="Contoh: 1" 
                        ...
                        value="{{ old('order_number', $act->order_number) }}" required>
                ...
                <div style="display: flex; gap: 10px; margin-top: 30px;">
                    ...
                    <button type="submit" class="btn btn-primary" style="padding: 10px 20px; font-size: 14px; font-weight: 600; cursor: pointer; background-color: #667eea; color: white; border: none; border-radius: 4px;">Update Act</button>
                </div>
            </form>
        </div>
    </div>
