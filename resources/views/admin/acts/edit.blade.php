...
@section('page-title', 'Edit Act')

@section('title', 'Admin - Edit Act')

@section('content')
    <div class="card">
        <div class="card-header">
            <h2 class="card-title">Edit Act</h2>
        </div>
        ...
        <div class="admin-card-body">
            ...
            <form action="{{ route('admin.acts.update', $act->id) }}" method="POST" class="admin-form">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select id="chapter_id" name="chapter_id" class="form-select @error('chapter_id') is-invalid @enderror" required>
                        ...
                        @foreach ($chapters as $chapter)
                            <option value="{{ $chapter->id }}" {{ old('chapter_id', $act->chapter_id) == $chapter->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror"
                        placeholder="Contoh: Act 1 - Pengenalan" value="{{ old('name', $act->name) }}" required>
                    @error('name')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" class="form-control @error('description') is-invalid @enderror"
                        placeholder="Masukkan deskripsi act..." rows="5">{{ old('description', $act->description) }}</textarea>
                    @error('description')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="order_number" name="order_number" class="form-control @error('order_number') is-invalid @enderror"
                        placeholder="Contoh: 1" value="{{ old('order_number', $act->order_number) }}" required>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn btn-primary">Update Act</button>
                </div>
            </form>
        </div>
    </div>
