...
@section('page-title', 'Tambah Chapter')
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h2 class="card-title">Tambah Chapter Baru</h2>
        </div>
        ...
        <div class="admin-card-body">
            ...
            <form action="{{ route('admin.chapters.store') }}" method="POST" class="admin-form">
                @csrf

                <div class="form-group">
                    <label for="name">Nama Chapter <span class="required">*</span></label>
                    <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror"
                        placeholder="Contoh: Chapter 1 - Pengenalan" value="{{ old('name') }}" required>
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" class="form-control @error('description') is-invalid @enderror"
                        placeholder="Masukkan deskripsi chapter..." rows="6" required>{{ old('description') }}</textarea>
                    @error('description')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
